...
def displayTeamDistributionsMatchup(Gamesdf,myteam,team2):
    ...
    density41 = ggplot(dff1, aes(x='Tempo', color='Team')) + ggsize(800, 550)+ geom_density(aes(fill='Team'), alpha=.3)+ scale_fill_brewer(type='seq')+ ggtitle("Pace/Tempo")+ geom_vline(xintercept=df1['Tempo'].median())+ geom_vline(xintercept=df['Tempo'].median(), color="red")+ geom_text(aes(label='y'), position=position_nudge(y=1), label_format='.1f', angle=45, color='#2b8cbe')
